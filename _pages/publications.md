---
layout: default
permalink: /publications/
title: Publications
nav: true
nav_order: 2
---

<div class="post">
  <div class="header-bar" style="text-align: center; margin-bottom: 2rem;">
    <h1 style="color: var(--global-theme-color); font-weight: bold;">Publications</h1>
    # <h2>Empirical research on threat, environment, and behavioral risk.</h2>
  </div>

  <p style="text-align: center; font-size: 1.1rem; margin-bottom: 3rem;">
    <em>I am currently migrating my publication list to accessible PDFs. Check back soon!</em>
  </p>

  <div class="publications">
    {% bibliography %}
  </div>
</div>
